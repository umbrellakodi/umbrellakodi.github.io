...
class Offcloud:
	...
	def _request(self, method, path, params=None, data=None):
		...
		if 'not_available' in r:
			reason = NOT_AVAILABLE.get(r.get('not_available')).lower()
			control.notification(reason)
		elif 'error' in r:
			reason = '\n%s\n%s' % (r.get('error'), full_path)
			log_utils.log('offcloud error %s' % reason)
		return r
	...
	def add_uncached_torrent(self, magnet_url, pack=False):
		...
		control.hide()
		if not result: control.okDialog(title=getLS(40018), message=getLS(33586))
		else: control.okDialog(title='default', message=getLS(40017) % 'Offcloud')
	...
	def auth(self):
		...
		control.setSetting('offcloudusername', user_id)
	...
	def remove_auth(self):
		try:
			...
			control.setSetting('offcloudusername', '')
	...
	def valid_url(self, host):
		...
		try:
			...
			if not self.hosts['Offcloud']: return False
			if any(host in item for item in self.hosts['Offcloud']): return True
			return False
		except: log_utils.error()
	...
	def get_hosts(self):
		...
		try:
			...
			hosts_dict['Offcloud'] = list(set(hosts))
		except: log_utils.error()
		return hosts_dict