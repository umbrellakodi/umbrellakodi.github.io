	...
	def _get(self, url, fail_check=False, token_ck=False):
		...
		try:
			...
			if 'Temporarily Down For Maintenance' in response.text:
				if self.server_notifications and not control.playerWindow.getProperty('umbrella.preResolving'): control.notification(message='Real-Debrid Temporarily Down For Maintenance', icon=rd_icon)
			...
			if self.server_notifications and not control.playerWindow.getProperty('umbrella.preResolving'): control.notification(message='Failed to connect to Real-Debrid', icon=rd_icon)
	...
	def _post(self, url, data):
		...
		try:
			...
			if any(value in str(response) for value in ('bad_token', 'Bad Request')):
				...
				if self.server_notifications and not control.playerWindow.getProperty('umbrella.preResolving'): control.notification(message=message, icon=rd_icon)
