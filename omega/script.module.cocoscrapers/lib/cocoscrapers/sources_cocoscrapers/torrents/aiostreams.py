...
class source:
	...
	def __init__(self):
		...
		instance_id = int(getSetting('aiostreams.instance', '0'))
		if instance_id == len(public_instances): self.base_link = getSetting('aiostreams.custom_url').strip().rstrip('/')
		else: self.base_link = public_instances[instance_id]
		self.movieSearch_link = "/api/v1/search"
		self.tvSearch_link = "/api/v1/search"
		self.min_seeders = 0
		username, password = getSetting('aiostreams.username'), getSetting('aiostreams.password')
	...
	def sources(self, data, hostDict):
		...
		try:
			...
			response_data = results.json()['data']
			files = response_data['results']
			errors = response_data.get('errors')
			if errors: log_utils.log('AIOSTREAMS errors: %s' % errors, level=log_utils.LOGWARNING)
			log_utils.log('AIOSTREAMS %ss, %s results' % (round(results.elapsed.total_seconds(), 3), len(files)), level=log_utils.LOGDEBUG)
